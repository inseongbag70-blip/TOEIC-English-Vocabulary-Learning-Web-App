...
function home(){
  ...
  ${review.length?`<section class="card" style="margin-top:13px"><h2>완료한 세트 복습</h2><p class="muted">이미 맞힌 단어도 다시 돌아가 학습·테스트할 수 있습니다.</p><div class="review-list">${review.map(i=>`<button class="review-chip" onclick="showLearn(${i})">${i+1}세트 (${GS[i][0].number}~${GS[i][GS[i].length-1].number})</button>`).join('')}</div><div class="actions"><button class="btn green" onclick="startReviewQuiz()">완료 단어 랜덤 복습 테스트</button></div></section>`:''}
...
function startQuiz(g,review){const ws=review?shuffle(GS.flat()).slice(0,5):(GS[g]||GS[0]); if(state.level===2)showLevel2Quiz(ws,review);else showLevel1Quiz(ws,g,review)}
...
function recordResult(score,total,wrong){state.quizzes++;state.correct+=score;state.answered+=total;wrong.forEach(w=>{const k=String(w.number);if(!state.wrong[k])state.wrong[k]={...w,wrong_count:0};state.wrong[k].wrong_count++});if(total===5 && score===5 && window.currentQuiz.type!=='level2' && !window.currentQuiz.review){const g=window.currentQuiz.g;if(g!==undefined&&!state.completed.includes(g))state.completed.push(g)}save()}
